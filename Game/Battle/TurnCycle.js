class TurnCycle {
  ...
  async turn() {
    ...
    if (submission.replacement) {
      ...
      await this.onNewEvent({
        type: "textMessage",
        text: `git ve şunları al ':, ${submission.replacement.name}!`
      })
      this.nextTurn();
      return;
    }
    ...
    if (targetDead) {
      await this.onNewEvent({ 
        type: "textMessage", text: `${submission.target.name} kaybetti!`
      })
    ...
    if (targetDead) {
      ...
      await this.onNewEvent({
        type: "textMessage",
        text: `${replacement.name} ortaya çıktı`
      })
    }
  ...
  async init() {
    await this.onNewEvent({
      type: "textMessage",
      text: `${this.battle.enemy.name} savaş istiyor!`
    })
